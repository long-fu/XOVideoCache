...
//        debugPrint("接收数据",data.count)
    }
    
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        completionHandler(Foundation.URLSession.ResponseDisposition.allow)
        debugPrint("开始响应数据",response.url ?? "error url is null" ,response.expectedContentLength,response.mimeType ?? "mimeType is null")
        mediaData = Data()
...
//        super.init(asset: AVAsset(url: url), automaticallyLoadedAssetKeys: nil)
    ...
    init(data: Data, mimeType: String, fileExtension: String) {
        ...
        addObserver(self, forKeyPath: "loadedTimeRanges", options: NSKeyValueObservingOptions.new, context: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(playbackStalledHandler), name:NSNotification.Name.AVPlayerItemPlaybackStalled, object: self)
